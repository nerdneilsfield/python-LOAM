...
class LOAMOptimizer:
    ...
    def optimize(self):
        opt_solution = least_squares(self.resid_function,
                                     np.zeros((6, )),
                                     xtol=1e-10,
                                     loss='huber')
    ...
    def resid_function(self, x):
        ...
        nu = np.linalg.norm(np.cross(aligned_edges - self.edge_factors[1],
                            ...
                            axis=1)
        de = np.linalg.norm(self.edge_factors[1] - self.edge_factors[2],
                            axis=1)
        ...
        surface_resid = np.abs(
            matrix_dot_product(aligned_surface - self.surface_factors[1],
                               normal))
    ...
    def optimize_2(self):
        opt_solution = least_squares(self.resid_function_2,
                                     np.zeros((6, )),
                                     loss='huber')
        T = mrob.geometry.SE3(opt_solution.x)
        return T
