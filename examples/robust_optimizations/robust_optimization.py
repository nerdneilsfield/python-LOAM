...
if __name__ == "__main__":
    ...
    with open(edge_corresp_file, newline="") as csvfile:
        ...
        for row in reader:
            edge_corresp_set.append(
                (point_from_str(row[0]), point_from_str(row[1]),
                 point_from_str(row[2])))
    ...
    with open(plane_corresp_file, newline="") as csvfile:
        reader = csv.reader(csvfile, delimiter=",")
        for row in reader:
            plane_corresp_set.append((
                point_from_str(row[0]),
                point_from_str(row[1]),
                point_from_str(row[2]),
    ...
    T, inlier_errors, edge_inliers, plane_inliers = solver.fit(
        edge_corresp_set, plane_corresp_set)
    ...
    o3d.visualization.draw_geometries([
        get_pcd_from_numpy(pcd1[0], color=[1, 0, 0]),
